...
- erik's home-manager surface is owned by the dotfiles repo, and two pieces of it are supplied from here rather than duplicated:
  - dotfiles selects a host profile by importing the host file itself (`users/erik/hades.nix`), which pulls in `users/erik/default.nix`.
    `machines/hades/configuration.nix` imports that path rather than `homeModules.erik`, which is only the default; importing both doubles it up.
