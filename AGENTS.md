...
- erik's home-manager surface is owned by the dotfiles repo, and two pieces of it are supplied from here rather than duplicated:
  ...
  - The apiserver VIP `10.0.69.100` is deliberately *not* in `hosts.nix`: that file is mapped 1:1 onto `internet` clan machines in `clan.nix`, so a non-machine key would break eval. It is the default of `dotfiles.kubernetes.rosequartz.server`.
