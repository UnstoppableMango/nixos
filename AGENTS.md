...
- erik's home-manager surface is owned by the dotfiles repo, and two pieces of it are supplied from here rather than duplicated:
  - dotfiles composes a host out of its `home/` (erik's identity) plus the profiles that host wants, and exports the result as `homeModules.hades` (`hosts/hades.nix`).
    `machines/hades/configuration.nix` imports that rather than `homeModules.erik`, which is only the `home/` half; importing both doubles it up.
    The host file leaves options declared elsewhere unset, so this repo supplies the same modules dotfiles' own `homeConfigurations` do: stylix, nixvim, nix2git, and sops-nix.
  - `sops.age.keyFile` comes from dotfiles' `modules/sops`. Nothing here sets it, but that module declares the option without importing sops-nix, so hades imports `sops-nix.homeManagerModules.sops`.
  - The rosequartz kubeconfig is generated by dotfiles' `modules/kubernetes/rosequartz`. `machines/hades/configuration.nix` supplies only what is repo-local: the CA at `vars/shared/rosequartz-ca/crt/value`, the admin cert/key paths, `currentContext`, and the `sops.templates` sink that keeps `~/.kube/config` a real 0600 file. Shape (contexts, VIP, dex OIDC exec block) is shared with darter.
...
| apollo        | ASUS board, firmware mode unverified | Basement server; clan-managed; not yet in rosequartz; takes castor's dual-mode grub |
