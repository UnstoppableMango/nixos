...
| `flux.nix` | Flux bootstrap via `inputs.inoculant` (`cluster.rosequartz.fluxBootstrap.enable`) |
...
- `flux.nix` bootstraps Flux via `inputs.inoculant`'s NixOS module (`cluster.rosequartz.fluxBootstrap.enable`, set unconditionally by `default.nix`'s `roles.control-plane` `perInstance`). It follows the same `clusterAdmin` pattern as `inoculant.nix` below, reusing `admin-cert` rather than minting a dedicated cert. Flux's manifests arrive via `manifestFiles` (rendered YAML from the `a2b` flux lib, which shells out to the real `flux` CLI) rather than structured `manifests`, so Nix can't auto-derive the bootstrap init container's `--allow` GVK scoping from them. `flux.nix` fills that gap with `additionalAllowedGVKs`, computed via a small IFD derivation that walks every YAML document in the rendered manifests with `yq-go`/`jq` and extracts the distinct `apiVersion`/`kind` pairs actually present.
- `inoculant.nix` is the coredns-only integration (`cluster.rosequartz.coredns.enable`). It reuses `admin-cert` (already defined by `kubeconfig.nix`, `system:masters`/client profile) for `services.kubernetes.inoculant.clusterAdmin` rather than minting a dedicated cert. `cluster.rosequartz.coredns.manifests` defaults to `config.services.kubernetes.addonManager.addons` (populated by nixpkgs' `addons.dns` module, which is mkDefault-enabled but never applied since `addonManager.enable = false`) — deliberately excludes `addonManager.bootstrapAddons`, which is RBAC for the `system:kube-addon-manager` principal (a service rosequartz never runs), not coredns. `default.nix`'s `roles.control-plane` `perInstance` sets `coredns.enable = true` unconditionally, so it's on for every control-plane node. `flux.nix` and `inoculant.nix` both define `services.kubernetes.inoculant.manifests`/`manifestFiles`/`clusterAdmin` — NixOS merges these additively as long as they don't redefine the same manifest key with different content, which is why `flux.nix` doesn't set `manifests` itself (that would re-apply coredns's addon manifests a second time, unpatched, and conflict with `inoculant.nix`'s node-affinity-patched copy). Flannel bootstrap via inoculant is a planned follow-up.
