...
| `coredns.nix` | CoreDNS manifests/patching (`cluster.rosequartz.coredns.enable`); contributes `services.kubernetes.inoculant.manifests` |
| `flux.nix` | Flux manifest building (`cluster.rosequartz.flux.enable`); contributes `services.kubernetes.inoculant.manifestFiles`/`additionalAllowedGVKs` |
| `inoculant.nix` | Inoculant integration layer: imports `inputs.inoculant.nixosModules.default` and defines `services.kubernetes.inoculant.clusterAdmin` in one place |
...
- `coredns.nix` (`cluster.rosequartz.coredns.enable`, set unconditionally by `default.nix`'s `roles.control-plane` `perInstance`) and `flux.nix` (`cluster.rosequartz.flux.enable`, same) are structurally symmetric: each owns its own manifest data and `enable` option, and each contributes `services.kubernetes.inoculant.enable = true` plus either `manifests` (coredns) or `manifestFiles`/`additionalAllowedGVKs` (flux) under its own `mkIf`. Neither defines `clusterAdmin` or imports the inoculant flake module — that's `inoculant.nix`'s job, once, so there's a single source of truth for the cert. NixOS merges the `manifests`/`manifestFiles`/`additionalAllowedGVKs` contributions additively as long as they don't redefine the same key with different content, which is why `flux.nix` only sets `manifestFiles` (never `manifests`) — setting `manifests` there would re-apply coredns's addon manifests a second time, unpatched, and conflict with `coredns.nix`'s node-affinity-patched copy.
- `flux.nix`'s manifests arrive via `manifestFiles` (rendered YAML from the `a2b` flux lib, which shells out to the real `flux` CLI) rather than structured `manifests`, so Nix can't auto-derive the bootstrap init container's `--allow` GVK scoping from them. `flux.nix` fills that gap with `additionalAllowedGVKs`, computed via a small IFD derivation that walks every YAML document in the rendered manifests with `yq-go`/`jq` and extracts the distinct `apiVersion`/`kind` pairs actually present.
- `coredns.nix`'s `cluster.rosequartz.coredns.manifests` defaults to `config.services.kubernetes.addonManager.addons` (populated by nixpkgs' `addons.dns` module, which is mkDefault-enabled but never applied since `addonManager.enable = false`) — deliberately excludes `addonManager.bootstrapAddons`, which is RBAC for the `system:kube-addon-manager` principal (a service rosequartz never runs), not coredns.
- `inoculant.nix` is the inoculant integration layer, not a coredns/flux-specific module: it imports `inputs.inoculant.nixosModules.default` and defines `services.kubernetes.inoculant.clusterAdmin`, reusing `admin-cert` (already defined by `kubeconfig.nix`, `system:masters`/client profile) rather than minting a dedicated cert. Flannel bootstrap via inoculant is a planned follow-up, and should follow the same pattern (its own file contributing manifests, `clusterAdmin` untouched).
